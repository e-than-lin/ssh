...
var type = 'default';//定义是否可选当前日期的前面的日期 或 后面的日期 默认情况都可选择 2014-07-11 由weiXH添加
var crrY=0,crrM=0,crrD=0; 
//选择日期 → 由 ouwt 2007-06-10 添加，通过 ID 来选日期
function SelectDateById(id,strFormat,x,y,type)
{
...
else{SelectDate(obj,strFormat,x,y,type)}
}
//选择日期 → 由 ouwt 2006-06-25 添加
function SelectDate(obj,strFormat,mode,x,y,type_)
{
leftX =(x == null) ? leftX : x;
topY =(y == null) ? topY : y;//自定义定位偏移量 2007-02-11 由 ouwt添
type =(type_ == null)?type:type_;
...
if(type !="default"){crrY=date.getFullYear();crrM=date.getMonth();crrD=date.getDate();}
...
if(mode!=null && mode!=""){cal.type=mode;}
cal.show(obj);
}
/**//**//**//**//**//**//**//**
* 返回日期
...
"sel_bg" : "#FFCCCC", //已被选择的日期单元格背影色 2006-12-03 ouwt添加
...
mvAry[mvAry.length] = ' <div name="calendarForm" style="margin: 0px;border-style:outset;border-width:0pt;  background-color:#EFEFEF">';

mvAry[mvAry.length] = ' <table width="100%" border="0" cellpadding="0" cellspacing="1" style="font-size:12px;">';
mvAry[mvAry.length] = ' <tr >';
mvAry[mvAry.length] = ' <td align="left" width="1%"><input style="border: 1px solid ' + calendar.colors["input_border"] + ';background-color:' + calendar.colors["input_bg"] + ';width:14px;height:20px;';
...
mvAry[mvAry.length] = ' <td align="center" width="98%" nowrap="nowrap"><select name="calendarYear" id="calendarYear" style="font-size:12px;"></select><select name="calendarMonth" id="calendarMonth" style="font-size:12px;';
if(calendar.DateMode > pickMode["month"]){mvAry[mvAry.length] = 'display:none;';}//pickMode 精确到年时隐藏“月”
mvAry[mvAry.length] = '"></select></td>';
mvAry[mvAry.length] = ' <td align="right"  width="1%"><input style="border: 1px solid ' + calendar.colors["input_border"] + ';background-color:' + calendar.colors["input_bg"] + ';width:13px;height:20px;';
if(calendar.DateMode > pickMode["month"]){mvAry[mvAry.length] = 'display:none;';}//pickMode 精确到年时隐藏“月”
...
mvAry[mvAry.length] = ' <input name="calendarClear" type="button" id="calendarClear" value="' + Calendar.language["clear"][this.lang] + '" style="border: 1px solid ' + calendar.colors["input_border"] + ';background-color:' + calendar.colors["input_bg"] + ';width:30px;height:20px;font-size:12px;cursor:pointer;"/>';
/**
 * modify by weixh
 * 只有默认类型才显示“今天”按钮
 */
if(type =="default"||type=="after_today"||type =="before_today"){
	mvAry[mvAry.length] = ' <input name="calendarToday" type="button" id="calendarToday" value="'+ Calendar.language["today"][this.lang]+'" style="border: 1px solid ' + calendar.colors["input_border"] + ';background-color:' + calendar.colors["input_bg"] + ';width:30px;height:20px;font-size:12px;cursor:pointer"/>';
}

if(calendar.DateMode!=4)
	{
mvAry[mvAry.length] = ' <input name="calendarOk" type="button" id="calendarOk" value="'+ Calendar.language["pickTxt"][this.lang]+'" style="border: 1px solid ' + calendar.colors["input_border"] + ';background-color:' + calendar.colors["input_bg"] + ';width:30px;height:20px;font-size:12px;cursor:pointer"/>';

}

mvAry[mvAry.length] = ' <input name="calendarClose" type="button" id="calendarClose" value="' + Calendar.language["close"][this.lang] + '" style="border: 1px solid ' + calendar.colors["input_border"] + ';background-color:' + calendar.colors["input_bg"] + ';width:30px;height:20px;font-size:12px;cursor:pointer"/>';
...
//var type = 'default';//定义是否可选当前日期的前面的日期 或 后面的日期 默认情况都可选择 2014-07-11 由weiXH添加
//var crrY=0,crrM=0,crrD=0; Before and after

if(type=='after'||type =="after_today"){//以后
	if(type=='after'&&today.getFullYear()>=crrY&&today.getMonth()>=crrM&&today.getDate()>crrD){
		calendar.ReturnDate(today.format(calendar.dateFormatStyle));
	}else if(type=='after_today'&&today.getFullYear()>=crrY&&today.getMonth()>=crrM&&today.getDate()>=crrD){
		calendar.ReturnDate(today.format(calendar.dateFormatStyle));	
	}
}else if(type=='before'||type =="before_today"){//以前
	if(type=='before'&&today.getFullYear()<=crrY&&today.getMonth()<=crrM&&today.getDate()<crrD){
		calendar.ReturnDate(today.format(calendar.dateFormatStyle));
	}else if(type=='before_today'&&today.getFullYear()<=crrY&&today.getMonth()<=crrM&&today.getDate()<=crrD){
		calendar.ReturnDate(today.format(calendar.dateFormatStyle));
	}
}else{
	calendar.ReturnDate(today.format(calendar.dateFormatStyle));	
...
{
var str = '<div id="calendarPanel" style="background-color: #FFFFFF;border: 1px solid #CCCCCC;width:175px;font-size:12px;"></div>';
if(document.all)
{
