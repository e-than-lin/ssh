...
TreeGrid = function($path,$indentation,$folderColumnIndex,$renderToR,$renderToL,$pre,$minDate,$maxDate,$format){
	...
	var vFormat=$format;
	var HeaderStart="<tr style='height:30px;'>";
	var indexStr="<th style='word-break: keep-all;white-space:nowrap;'>编号</th>";
	var preTH="<th style='word-break: keep-all;white-space:nowrap;'>前置工作</th>";
	var Headerendt="</tr>";
	var rownum = 0;
	var pre=$pre;
	var __root;
     ...
     this.addHeader  = function(headerAlign,width,headerText)
	{
		...
		h += "<th style='word-break: keep-all;white-space:nowrap;' align='" + (headerAlign  || "center") + "' >" + (headerText || "") + "</th>";
	    }
		 else
		{
		 
		 h += "<th style='word-break: keep-all;white-space:nowrap;' align='" + (headerAlign  || "center") + "' width='" + (width || "") + "px'>" + (headerText || "") + "</th>";
	     }
		 f+="<td ></td>";
		 number++;
	 }
    ...
    drowHeader= function()
	{
	  if(vindex==1)
	  {
		  ...
		  f="<tr style='height:31px;' ><td ></td>"+f;
		  number++;
	  }
	  else
	  {
		  h=HeaderStart+h;
		  f="<tr style='height:31px;'>"+f;
	  }
	  if(pre==1)
	  {
		  h=h+preTH;
	...
	this.show = function(){
		...
		rs += "<table   id='dateTable' class='list_table'><tr ><th colspan="+number+"  style='height:26px;'></th></tr> ";
   ...
   drowRRowData = function(obj,_level,orderStr,order,_pid){
			...
			else
		   {
            ...
            if(status%2==0)
	         {
			  d += "<tr  style='height:31px;' selseName='date' orders="+orders+" wid='"+obj.id+"'  id='TR" + pid + "' class='td_even'  onmouseover='over(this);'   onmouseout='out(this);'  othClass='td_even'  width='" + ((obj.width=="")?"":(obj.width+"pm")) + "'  pid='" + ((_pid=="")?"":("TR"+_pid)) + "' tropen='Y'   rowIndex='" + rownum++ + "'>";
            
			}
			else
	     {
			 d += "<tr  style='height:31px;' selseName='date' orders="+orders+" wid='"+obj.id+"' id='TR" + pid + "'  othClass='td_odd'   onmouseover='over(this);'  onmouseout='out(this);'         class='td_odd'   width='" + ((obj.width=="")?"":(obj.width+"pm")) + "'  pid='" + ((_pid=="")?"":("TR"+_pid)) + "' tropen='Y'   rowIndex='" + rownum++ + "'>";
           ...
           if(vindex==1)
           {
        	   d += "<td style='word-break: keep-all;white-space:nowrap;'  >"+orders+"</td>";
           }
			...
			for(var j=0;j<obj.columns.length;j++)
		    {
				...
				{
				d += "<td  style='word-break: keep-all;white-space:nowrap;' align='left'";
				}
				else
				{
				
				d += "<td style='word-break: keep-all;white-space:nowrap;'  align='" + (col.dataAlign || "left") + "'";
				}
				...
				//	d += " style='display:inline;padding-left:" + (parseInt((indentation || "20"))*(_level-1)) + "px;'> ";
					d += "><img src='" + defaultLeafIcon + "' style='height:25px;width:" + (parseInt((indentation || "20"))*(_level-1)) + "px;'   class='image_nohand'>";
				}else{
					d += ">";
				}
			...
			if(pre==1)
			{
				d +="<td  style='word-break: keep-all;white-space:nowrap;height:27px;' width='60px' id='PRE" + pid + "' style='word-WRAP: break-word'></td>";
       ...
       if(obj.pMile==1) {
    	   ...
    	   if (vFormat='day')
           {
    		   leftNum=10;
                ...
                '<TR  style=" HEIGHT: 30px;" id="childrow_' + vID + '"  >' + vItemRowStr + '</TR></TABLE></DIV>';
		...
		//日期格式
	      ...
	      rownum=0;
	         ...
	         // vDayWidth = (vColWidth / vColUnit) + (1/vColUnit);
	          ...
	          ls='<table style="border:0;width: ' + vChartWidth + 'px;"   class="list_table"> <TR style="HEIGHT: 26px;padding: 0px;" >';
	            ...
	            {	
	               ...
	               vStr = vStr.substring(2,4);
	     	         ...
	     	         if(vFormat == 'day')
	               {
	   			      ls += '<th class=gdatehead style="FONT-SIZE: 12px; " align=center colspan=7>' +
	               ...
	               {
	     		         ls += '<th class=gdatehead align=center style="FONT-SIZE: 12px;" width='+vColWidth+10+'px>`'+ vStr + '</th>';
	            ...
	            ls += '</TR><TR style="HEIGHT: 30px;border: 2px solid solid #8BB6EF;padding: 0px;" >';
	            ...
	            {	
	                if(vFormat == 'day' )
	                {
	                  ...
	                  if(vTmpDate.getDay() % 6 == 0) {
	                     ...
	                     vItemRowStr  += '<td class="gheadwkend" style="width: '+vColWidth+10+'px;BORDER-TOP: #efefef 0px solid; FONT-SIZE: 12px; BORDER-LEFT: #efefef 1px solid; cursor: default;"  bgcolor=#' + vWeekendColor + ' align=center><div style="width: '+vColWidth+'px">&nbsp</div></td>';
	                  }
	                  else {
	                     vDateRowStr += '<td class="ghead" style="FILTER: progid:DXImageTransform.Microsoft.Gradient(gradientType=0,startColorStr=#FDFDFD,endColorStr=#DFDFDF);width: 30px;BORDER-TOP: #efefef 1px solid; FONT-SIZE: 12px;   BORDER-LEFT: #efefef 1px solid;"  bgcolor=#' + vWeekdayColor + ' align=center><div style="width: '+vColWidth+'px">' + vTmpDate.getDate() + '</div></td>';
	                     if( formatDateStr(vCurrDate,'mm/dd/yyyy') == formatDateStr(vTmpDate,'mm/dd/yyyy')) 
	                        vItemRowStr += '<td class="ghead" style="width: '+vColWidth+10+'px;BORDER-TOP: #efefef 1px solid; FONT-SIZE: 12px; width: '+vColWidth+'px;  BORDER-LEFT: #efefef 1px solid; cursor: default;"  bgcolor=#' + vWeekdayColor + ' align=center><div style="width: '+vColWidth+'px">&nbsp&nbsp</div></td>';
	                     else
	                        vItemRowStr += '<td class="ghead" style="width: '+vColWidth+10+'px;BORDER-TOP: #efefef 1px solid;FONT-SIZE: 12px; BORDER-LEFT: #efefef 1px solid; cursor: default;"  align=center><div style="width: '+vColWidth+'px">&nbsp&nbsp</div></td>';
	                  }
	   	         ...
	   	         else if(vFormat == 'week')
	               {
	                  ...
	                  if(vNxtDate <= vMaxDate) {
	                     ...
	                     if( vCurrDate >= vTmpDate && vCurrDate < vNxtDate ) 
	                        vItemRowStr += '<td class="ghead" style="width: '+vColWidth+10+'px;BORDER-TOP: #efefef 1px solid; FONT-SIZE: 12px; BORDER-LEFT: #efefef 1px solid;" bgcolor=#' + vWeekdayColor + ' align=center><div style="width: '+vColWidth+'px">&nbsp&nbsp</div></td>';
	                     else
	                        vItemRowStr += '<td class="ghead" style="width: '+vColWidth+10+'px;BORDER-TOP: #efefef 1px solid; FONT-SIZE: 12px; BORDER-LEFT: #efefef 1px solid;" align=center><div style="width: '+vColWidth+'px">&nbsp&nbsp</div></td>';
	                     ...
	                     if( vCurrDate >= vTmpDate && vCurrDate < vNxtDate ) 
	                        vItemRowStr += '<td class="ghead" style="width: '+vColWidth+10+'px;BORDER-TOP: #efefef 1px solid; FONT-SIZE: 12px; BORDER-LEFT: #efefef 1px solid; BORDER-RIGHT: #efefef 1px solid;" bgcolor=#' + vWeekdayColor + ' align=center><div style="width: '+vColWidth+'px">&nbsp&nbsp</div></td>';
	                     else
	                        vItemRowStr += '<td class="ghead" style="width: '+vColWidth+10+'px;BORDER-TOP: #efefef 1px solid; FONT-SIZE: 12px; BORDER-LEFT: #efefef 1px solid; BORDER-RIGHT: #efefef 1px solid;" align=center><div style="width: '+vColWidth+'px">&nbsp&nbsp</div></td>';
	  	        ...
	  	        drowLRowData(rowPArray[i],vTmpDate,vMinDate,vChartWidth,vItemRowStr,vDateRowStr,vDateDisplayFormat,vTaskLeft,vTaskRight,vDayWidth+1,vColUnit,'');
	        			...
	        			for(var i=0;i<10;i++)
	        			{
	        				 ls += '<DIV><TABLE  class="list_table"  style="position:relative; border:0px ; width: ' + vChartWidth + 'px;" cellSpacing=0 cellPadding=0 border=0>' +
	        	                '<TR style=" HEIGHT: 30px;" >' + vItemRowStr + '</TR></TABLE></DIV>';
	        			}
