...
//==================================================
// Node object
function Node(id, pid, cname, cvalue, cshow, cchecked, cdisabled, url, title, target, icon, iconOpen, open,showFlag) {
	this.id = id;
	this.pid = pid;
	//chechbox的名称
	this.cname = cname;
...
// Tree object type=0表示单纯的树，1：表示单选框，2：表示复选框
function dTree(objName,_iconPath,type) {
	this.config = {
		target					: null,
		folderLinks			: true,
		useSelection		: true,
...
dTree.prototype.add = function(id, pid, cname, cvalue, cshow, cchecked, cdisabled, url, title, target, icon, iconOpen, open,showFlag) {
	this.aNodes[this.aNodes.length] = new Node(id, pid, cname, cvalue, cshow, cchecked, cdisabled, url, title, target, icon, iconOpen, open,showFlag);
};
...
dTree.prototype.node = function(node, nodeId) {
	...
	if(node.pid == this.root.id){
		...
		if(0==this.type){
			str += node.cshow;
		}else if('single'==this.type){
			/**组装checkbox开始*/
			checkboxSyntax = "<input type='radio' desc='" + node.cshow + "' name='" + node.cname + "' id='" + node.cname + "_" + node.id + "' value='" + node.cvalue + "'";
			//是否被选中
			if("true"==node.cchecked){
		...
		}else if("double"==this.type){
			//===============================================
			//2006-05-11 将原链接的节点修改为 checkbox
			//===============================================
			//str += node.name;
			if("true"==node.showFlag){
				/**组装checkbox开始*/
				checkboxSyntax = "<input type='checkbox' desc='" + node.cshow + "' name='" + node.cname + "' id='" + node.cname + "_" + node.id + "' value='" + node.cvalue + "'";
				//是否被选中
				if("true"==node.cchecked){
					checkboxSyntax += " checked ";
				}
...
dTree.prototype.getSelected = function() {
	var sn = this.getCookie('css' + this.obj);
	return (sn) ? sn : null;
};
...
dTree.prototype.clearCookie = function() {
	...
	this.setCookie('coo'+this.obj, 'cookieValue', yesterday);
	this.setCookie('css'+this.obj, 'cookieValue', yesterday);
};
...
dTree.prototype.updateCookie = function() {
	...
	this.setCookie('coo' + this.obj, str);
};

// [Cookie] Checks if a node id is in a cookie
dTree.prototype.isOpen = function(id) {
	var aOpen = this.getCookie('coo' + this.obj).split('.');
