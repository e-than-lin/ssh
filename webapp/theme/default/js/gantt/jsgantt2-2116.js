...
TreeGrid = function($path,$indentation,$folderColumnIndex,$renderToR,$renderToL,$pre,$minDate,$maxDate,$format){
    ...
    drowHeader= function()
	{
	  if(vindex==1)
	  {
		  ...
		  f="<tr><td style='height:27px;'></td>"+f;
	  }
	  else
	  {
		  h=HeaderStart+h;
	...
	this.show = function(){
		rs += "<table  style='width:850px;'  id='dateTable' > ";
		rs += "<tr><td style='height:20px;FILTER: progid:DXImageTransform.Microsoft.Gradient(gradientType=0,startColorStr=#FDFDFD,endColorStr=#DFDFDF);'></td></tr></table>";
		rs += "<table  style='width:850px;'  id='dateTable' class='list_table'> ";
	    drowHeader();
	    rs +=h;
		drowRData();
		rs +=d;
   ...
   drowRRowData = function(obj,_level,orderStr,order,_pid){
			...
			else
		   {
            ...
            if(status%2==0)
	         {
			  d += "<tr  selseName='date' orders="+orders+" wid='"+obj.id+"'  id='TR" + pid + "' class='td_even'  onmouseover='over(this);'   onmouseout='out(this);'  othClass='td_even'  width='" + ((obj.width=="")?"":(obj.width+"px")) + "'  pid='" + ((_pid=="")?"":("TR"+_pid)) + "' open='Y'  rowIndex='" + rownum++ + "'>";
            
			}
			else
	     {
			 d += "<tr selseName='date' orders="+orders+" wid='"+obj.id+"' id='TR" + pid + "'  othClass='td_odd'   onmouseover='over(this);'  onmouseout='out(this);'         class='td_odd'   width='" + ((obj.width=="")?"":(obj.width+"px")) + "'  pid='" + ((_pid=="")?"":("TR"+_pid)) + "' open='Y'  rowIndex='" + rownum++ + "'>";
           ...
           if(vindex==1)
           {
        	   d += "<td width='30px'  style='height:27px;'>"+orders+"</td>";
           }
			...
			for(var j=0;j<obj.columns.length;j++)
		    {
				...
				{
				d += "<td  style='height:27px;' align='left'";
				}
				else
				{
				
				d += "<td  style='height:27px;' align='" + (col.dataAlign || "left") + "'";
				}
				...
				//层次缩进
				if(j==folderColumnIndex){
					d += " style='text-indent:" + (parseInt((indentation || "20"))*(_level-1)) + "px;'> ";
				}else{
					d += ">";
				}
	      ...
	      rownum=0;
	         ...
	         // vDayWidth = (vColWidth / vColUnit) + (1/vColUnit);
	          ...
	          ls='<table style="border:0;width: ' + vChartWidth + 'px;"   class="list_table"> <TR style="padding: 1px;padding:1px;" >';
	            ...
	            {	
	                if(vFormat == 'day' )
	                {
	                  ...
	                  if(vTmpDate.getDay() % 6 == 0) {
	                     ...
	                     vItemRowStr  += '<td class="gheadwkend" style="height:27px;width: '+vColWidth+10+'px;BORDER-TOP: #efefef 0px solid; FONT-SIZE: 12px; BORDER-LEFT: #efefef 1px solid; cursor: default;"  bgcolor=#' + vWeekendColor + ' align=center><div style="width: '+vColWidth+'px">&nbsp</div></td>';
	                  }
	                  else {
	                     ...
	                     if( formatDateStr(vCurrDate,'mm/dd/yyyy') == formatDateStr(vTmpDate,'mm/dd/yyyy')) 
	                        vItemRowStr += '<td class="ghead" style="height:27px;width: '+vColWidth+10+'px;BORDER-TOP: #efefef 0px solid; FONT-SIZE: 12px; width: '+vColWidth+'px;  BORDER-LEFT: #efefef 1px solid; cursor: default;"  bgcolor=#' + vWeekdayColor + ' align=center><div style="width: '+vColWidth+'px">&nbsp&nbsp</div></td>';
	                     else
	                        vItemRowStr += '<td class="ghead" style="height:27px;width: '+vColWidth+10+'px;BORDER-TOP: #efefef 0px solid;FONT-SIZE: 12px; BORDER-LEFT: #efefef 1px solid; cursor: default;"  align=center><div style="width: '+vColWidth+'px">&nbsp&nbsp</div></td>';
	                  }
	   	         ...
	   	         else if(vFormat == 'week')
	               {
	                  ...
	                  if(vNxtDate <= vMaxDate) {
	                     ...
	                     if( vCurrDate >= vTmpDate && vCurrDate < vNxtDate ) 
	                        vItemRowStr += '<td class="ghead" style="height:27px;width: '+vColWidth+10+'px;BORDER-TOP: #efefef 1px solid; FONT-SIZE: 12px; BORDER-LEFT: #efefef 1px solid;" bgcolor=#' + vWeekdayColor + ' align=center><div style="width: '+vColWidth+'px">&nbsp&nbsp</div></td>';
	                     else
	                        vItemRowStr += '<td class="ghead" style="height:27px;width: '+vColWidth+10+'px;BORDER-TOP: #efefef 1px solid; FONT-SIZE: 12px; BORDER-LEFT: #efefef 1px solid;" align=center><div style="width: '+vColWidth+'px">&nbsp&nbsp</div></td>';

	                  } else {
	                     vDateRowStr += '<td class="ghead" style="FILTER: progid:DXImageTransform.Microsoft.Gradient(gradientType=0,startColorStr=#FDFDFD,endColorStr=#DFDFDF);width: '+vDayWidth+'px;BORDER-TOP: #efefef 1px solid; FONT-SIZE: 12px; HEIGHT: 27px; BORDER-LEFT: #efefef 1px solid; bgcolor=#' + vWeekdayColor + ' BORDER-RIGHT: #efefef 1px solid;" align=center width:'+vColWidth+'px><div style="width: '+vColWidth+'px">' + (vTmpDate.getMonth()+1) + '/' + vTmpDate.getDate() + '</div></td>';
	                     if( vCurrDate >= vTmpDate && vCurrDate < vNxtDate ) 
	                        vItemRowStr += '<td class="ghead" style="height:27px;width: '+vColWidth+10+'px;BORDER-TOP: #efefef 1px solid; FONT-SIZE: 12px; BORDER-LEFT: #efefef 1px solid; BORDER-RIGHT: #efefef 1px solid;" bgcolor=#' + vWeekdayColor + ' align=center><div style="width: '+vColWidth+'px">&nbsp&nbsp</div></td>';
	                     else
	                        vItemRowStr += '<td class="ghead" style="height:27px;width: '+vColWidth+10+'px;BORDER-TOP: #efefef 1px solid; FONT-SIZE: 12px; BORDER-LEFT: #efefef 1px solid; BORDER-RIGHT: #efefef 1px solid;" align=center><div style="width: '+vColWidth+'px">&nbsp&nbsp</div></td>';
	           ...
	           ls+= '<DIV class=scroll3 id=rightside>';
	            if(rowPArray.length > 0)
	  	      {
	          for(var i=0;i<rowPArray.length;i++)
	  		 {
